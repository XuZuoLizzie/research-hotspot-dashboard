...
function heat(a){const journalImpact=a.reduce((m,x)=>{if(x.journal)m[x.journal]=Math.max(m[x.journal]??-Infinity,number(x.impact));return m},{}),js=Object.entries(journalImpact).sort((x,y)=>y[1]-x[1]||String(x[0]).localeCompare(String(y[0]),'zh-CN')).slice(0,10).map(x=>x[0]),fs=uniq(a,'future'),z=js.map(j=>fs.map(f=>a.filter(d=>d.journal===j&&d.future===f).length));plot('directionJournal',[{type:'heatmap',x:fs,y:js,z,colorscale:'YlGnBu',customdata:js.map(j=>fs.map(()=>journalImpact[j])),hovertemplate:'%{y}<br>IF %{customdata}<br>%{x}<br>%{z} 篇<extra></extra>'}],{margin:{l:200,r:15,t:10,b:145},xaxis:{tickangle:-35},yaxis:{autorange:'reversed'}});const e=$('directionJournal');e.removeAllListeners?.('plotly_click');e.on?.('plotly_click',v=>{$('journal').value=v.points[0].y;$('future').value=v.points[0].x;update()})}
...
function cards(a){const v=a.slice(0,120);$('articleCount').textContent=`显示 ${v.length} / ${a.length} 篇`;$('articles').classList.toggle('empty-state',!v.length);$('articles').innerHTML=v.length?v.map(x=>{const url=normalizeUrl(x);return `<article class="article"><div class="meta">${esc(x.date)} · ${esc(x.journal)} · 引用 ${x.citation} · IF ${x.impact||'NA'}</div><h3>${esc(x.title_chinese||x.title)}</h3>${x.title_chinese?`<h4>${esc(x.title)}</h4>`:''}<div><span class="pill">${esc(x.modality||'未分类')}</span><span class="pill">${esc(x.theme||'未分类')}</span>${x.watchlist_matched?'<span class="pill">Watchlist</span>':''}</div>${x.highlight?`<div class="article-highlight"><div class="highlight-label">研究亮点</div><div class="abstract-text">${esc(x.highlight)}</div></div>`:''}${url?`<p><a class="doi-link" href="${esc(url)}" target="_blank" rel="noopener noreferrer">DOI：${esc(x.doi||url)}</a></p>`:''}</article>`}).join(''):'当前筛选条件下没有文章。'}
