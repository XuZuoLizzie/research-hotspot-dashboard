...
function heat(a){
  ...
  a.forEach(x=>{
    if(!x.journal) return;
    const impact=number(x.impact);
    journalImpact[x.journal]=Math.max(journalImpact[x.journal]??0,impact);
  });
  const journals=Object.entries(journalImpact)
    .sort((x,y)=>y[1]-x[1]||String(x[0]).localeCompare(String(y[0]),'zh-CN'))
    .slice(0,10);
  const js=journals.map(x=>x[0]);
  const fs=uniq(a.filter(x=>js.includes(x.journal)),'future');
  const z=js.map(j=>fs.map(f=>a.filter(d=>d.journal===j&&d.future===f).length));
  const journalIf=Object.fromEntries(journals);
  plot('directionJournal',[{
    type:'heatmap',x:fs,y:js,z,colorscale:'YlGnBu',
    customdata:js.map(j=>fs.map(()=>journalIf[j])),
    hovertemplate:'%{y}<br>影响因子 %{customdata}<br>%{x}<br>%{z} 篇<extra></extra>'
  }],{
    margin:{l:230,r:15,t:10,b:145},
    xaxis:{tickangle:-35},
    yaxis:{categoryorder:'array',categoryarray:[...js].reverse()}
  });
...
function cards(a){
  const v=a.slice(0,120);
  $('articleCount').textContent=`显示 ${v.length} / ${a.length} 篇`;
  $('articles').classList.toggle('empty-state',!v.length);
  $('articles').innerHTML=v.length?v.map(x=>{
    const url=normalizeUrl(x);
    return `<article class="article">
      <div class="meta">${esc(x.date)} · ${esc(x.journal)} · 引用 ${x.citation} · IF ${x.impact||'NA'}</div>
      <h3>${esc(x.title_chinese||x.title)}</h3>
      ${x.title_chinese?`<h4>${esc(x.title)}</h4>`:''}
      <div>
        <span class="pill">${esc(x.modality||'未分类')}</span>
        <span class="pill">${esc(x.theme||'未分类')}</span>
        ${x.watchlist_matched?'<span class="pill">Watchlist</span>':''}
      </div>
      ${x.highlight?`<div class="highlight-text">${esc(x.highlight)}</div>`:''}
      ${url?`<p><a class="doi-link" href="${esc(url)}" target="_blank" rel="noopener noreferrer">DOI：${esc(x.doi||url)}</a></p>`:''}
    </article>`;
  }).join(''):'当前筛选条件下没有文章。';
}
